...
# Function to load assembly instructions from a file
...
def patch_branch_instruction(b_instruction, address, target_address):
    ...
    instruction_size = 4  # Assuming a standard instruction size
    offset = (target_address - (current_address + instruction_size)) // instruction_size

    # Ensure the offset is within a valid range for branch instructions
    if offset < -0x80000000 // instruction_size or offset > 0x7FFFFFFF // instruction_size:
...
def assemble_and_print(instructions, cheat=False):
    ...
    for address, instruction, comment in instructions:
        print(f"Processing instruction: {instruction} at address {address}")
        ...
        try:
            ...
            print(f"Assembly failed for instruction '{instruction}' at address {formatted_address}: {e}")
